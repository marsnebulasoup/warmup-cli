...
let vm = new Vue({
  ...
  methods: {
    ...
    async d2cWarmup(answers) {
      this.warmup(answers, false)
    },
    async b2bWarmup(answers) {
      this.warmup(answers, true);
    },
    async warmup(answers, login) {
      ...
      while (NODES.length < parseInt(answers.nodes)) {
        ...
        let warmup = new Warmup(node.driver, answers.urls, node.value, { login: login, email: answers.email, password: answers.password, url: answers.url });
