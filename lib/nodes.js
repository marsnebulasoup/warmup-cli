  ...
  Discover: class {
    ...
    async start() {
      while (this.nodes.length < this.node_count + 1) {
        ...
        let cookie = "";
        try {
          cookie = await driver.manage().getCookie(this.cookie_name);
        } catch(err) {
          console.log(chalk.red.bold(`\nERROR: No cookies named "${this.cookie_name}" were found. Please restart the warmup with the correct cookie name.\n`))
          process.exit()
        }
        cookie = cookie.value;
